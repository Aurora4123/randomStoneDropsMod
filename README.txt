A mod makes stone random drops with some common items added,
due to my poor texture drawing skill I refrenced some resoures created by others,
here is the refrenced texture and model resources list(Will be updated irregularly):
Modern Waystones Textures - Minecraft Resource Pack : https://modrinth.com/resourcepack/modern-waystones-textures
Waystones Resprite - Minecraft Resource Pack: https://modrinth.com/resourcepack/waystones-resprite
Faithful 32x Daggers - Minecraft Resource Pack : https://modrinth.com/resourcepack/faithful-32x-daggers
Part of weapons and tools from:
My plugins resource pack - Minecraft Resource Pack: https://modrinth.com/resourcepack/my-plugins-resource-pack
-------------------------------------------------------------
This is my first mod project for mod developing study, sorry for any shortcomings _(:зゝ∠)_
Pull requests and issues are welcome!
-------------------------------------------------------------
ATTENTION:
This project is licensed under the MIT License. See LICENSE.txt for details.
